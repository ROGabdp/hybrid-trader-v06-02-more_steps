...
RESULTS_PATH = os.path.join(PROJECT_PATH, 'results_backtest_v5_dca_hybrid_dynamic_n_kd_filter_queue')
...
class LeveragedSharedPoolBacktester:
    ...
    def run(self, df: pd.DataFrame, feature_cols: list) -> dict:
        ...
        for i in range(len(df)):
            ...
            # 當日追蹤變數 (Initialize before Step 0)
            day_action = 'HOLD'
            day_buy_conf = None
            day_sell_conf = None
            day_note = ''
            ...
            # =========== 0. 執行 Pending AI Buys (Queue Logic) ===========
            # 檢查是否有待買入隊列，若有，且今日收盤價 < 昨日收盤價 (日線收跌)，則執行一筆
            # Execution Price: Today Close
            
            # 使用列表副本遍歷，因為可能移除
            if self.pending_ai_orders:
                # 只有 "日線收跌" 才執行 (Close < Prev Close)
                # i > 0 check is implicit as loop starts at 0, but logic requires i-1
                is_down_day = False
                if i > 0:
                    prev_close = close_prices[i-1]
                    if price < prev_close: # price is current Close
                        is_down_day = True
                
                if is_down_day:
                    # 取出最早的一筆 (FIFO)
                    order = self.pending_ai_orders.pop(0) 
                    amount, gen_date, log_entry, buy_data_entry = order
                    
                    # 執行買入 @ Close Price
                    fund = amount
                    if yearly_pool >= fund:
                        yearly_pool -= fund
                        shares = int(fund / price) # Execute at Close
                        cost = shares * price
                        leftover = fund - cost
                        ...
                        # 完成 Log Entry (from generation day)
                        # We don't update the old log entry in the past daily_confidence, 
                        # instead we might want to log execution today?
                        # But log_entry passed here is the one created on signal generation day.
                        log_entry['action'] = 'QUEUED_EXEC' # Update status in history if reference is kept?
                        log_entry['note'] = f'Exec_on_DownDay_{date.date()}'

                        # 更新 Buy Signal Entry
                        buy_data_entry['price'] = price
                        buy_data_entry['shares'] = shares
                        buy_data_entry['cost'] = cost
                        # buy_data_entry needs to be added to self.ai_buy_signals?
                        # In original code, it was already appended to self.ai_buy_signals?
                        # Let's check generation logic below.
                        # Original generation logic appended to self.pending_ai_orders AND self.ai_buy_signals?
                        # No, I should check generation logic.
                        
                        self.ai_buy_signals.append(buy_data_entry) # Add to signals list UPON EXECUTION? 
                        # Better to add when generated but mark as pending?
                        # To allow correct metric calculation, usually signals list is for executed trades?
                        # I will assume I append here.
                        
                        day_action = 'BUY_EXEC'
                        day_bought_shares = shares
                        day_ai_leverage = current_leverage
                    else:
                        # Funds insufficient today
                        # Should we requeue? Or Cancel?
                        # Queue logic usually implies wait until funds available? 
                        # But simpler to Cancel if no funds.
                        # Or put back to front?
                        self.pending_ai_orders.insert(0, order) # Put back
                        # print(f"  [QUEUE] Funds insufficient, retry later.")
                else:
                    # Not down day, wait
                    pass
                    ...
                    if pass_dynamic_filter and pass_kd_filter:
                        ...
                        if fund > price: # Price here is Close, just for estimation
                            ...
                            buy_data = {
                                ...
                                'price': price, # Signal Price (Close)
                            ...
                            # Queue it!
                            # New structure: (amount, gen_date, log_entry, buy_data_entry)
                            # No need for limit_price as we check Close < Prev Close dynamically
                            self.pending_ai_orders.append((fund, date, log_entry, buy_data))
                            
                            # Note: day_action stays 'HOLD' for today summary, effectively 'QUEUED'
                             # day_action = 'QUEUED'
                             # day_bought_shares = 0
                             # day_ai_leverage = current_leverage
                    else:
            ...
            if self.pending_ai_orders:
                ...
                if is_down_day:
                    ...
                    if yearly_pool >= fund:
                        ...
                        # 資金不足，放回隊列前頭
                        self.pending_ai_orders.insert(0, order)
                        log_entry['action'] = 'QUEUED_NO_FUND'
                else:
                    # Not down day
                    pass
                ...
                if buy_action_pred[0] == 1:
                    ...
                    if pass_dynamic_filter:
                        ...
                        if fund > price:
                            ...
                            # Queue structure: (amount, gen_date, log_entry, buy_data_entry)
                            self.pending_ai_orders.append((fund, date, log_entry, buy_data))
                    else:
                        # 被濾網擋下
                        log_entry['action'] = 'FILTERED'
...
def generate_end_date_summary(bt1, twii_df, start_date, end_date, buy_model, sell_model, 
    ...
    # Queue Status and Advice
    pending_count = len(bt1.pending_ai_orders) if hasattr(bt1, 'pending_ai_orders') else 0
    
    if buy_signal == "BUY" and buy_conf > 0.5 and kd_pass and pass_dynamic_filter:
        lines.append(f"   📝 新增訂單: 已加入隊列 (Queued)")
        
    if pending_count > 0:
        lines.append(f"   ⚠️ 待買入隊列 (Queue): {pending_count} 筆訂單")
        lines.append(f"      👉 執行條件: 當日收盤 < 昨日收盤 (日線收跌)")
        lines.append(f"      👉 執行順序: FIFO (每日最多 1 筆)")
